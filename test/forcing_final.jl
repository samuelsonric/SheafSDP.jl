...
for run in 1:3
    t_off = @elapsed result_off = solve(prob, settings_off)
    t_on = @elapsed result_on = solve(prob, settings_default)
    push!(times_off, t_off)
    push!(times_on, t_on)
    @printf("  Run %d: OFF=%.1f ms (%d CG), ON=%.1f ms (%d CG)\n",
        run, t_off*1000, result_off.kkt_niter, t_on*1000, result_on.kkt_niter)
end

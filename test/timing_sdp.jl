...
for run in 1:3
    ...
    push!(times, t)
    @printf("Run %d: %.1f ms, %d IPM iters, %d total CG\n",
        run, t*1000, result.ipm_niter, result.kkt_niter)
end
@printf("Average: %.1f ms\n", mean(times)*1000)

# Run with verbose to get per-iteration breakdown
println("\nPer-iteration timing analysis:")
settings_verbose = SheafSDP.IPMSettings{Float64}(
    kkt=SheafSDP.UzawaSettings{Float64}(raug=1e6),
    feas_tol=1e-8, gap_tol=1e-8, itmax=100, verbose=true, refine_itmax=0
)
result = solve(prob, settings_verbose)

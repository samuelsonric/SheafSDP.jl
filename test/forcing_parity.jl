...
println("Forcing-term parity test: N=$N, n_i=$n_i")
println("="^60)

# Settings with force_tol=0 (should be same as old behavior)
...
# Settings with force_tol=1e-3 (new behavior, conservative)
settings_on = SheafSDP.IPMSettings{Float64}(
    kkt=SheafSDP.UzawaSettings{Float64}(raug=1e6),
    feas_tol=1e-8, gap_tol=1e-8, itmax=100, verbose=false,
    force_tol=1e-3  # ON (conservative)
)

# Warmup
println("Warmup...")
solve(prob, settings_off)
solve(prob, settings_on)

# Run with forcing OFF
println("\nforce_tol = 0 (OFF):")
result_off = solve(prob, settings_off)
println("  Status: $(result_off.status)")
println("  IPM iters: $(result_off.ipm_niter)")
println("  Total CG: $(result_off.kkt_niter)")
println("  Per-iteration CG:")
println("  iter |      μ      | pred | corr | total")
println("  -----|-------------|------|------|------")
for (i, row) in enumerate(result_off.history)
    @printf("  %4d | %11.3e | %4d | %4d | %5d\n", i, row.μ, row.npred, row.ncorr, row.npred + row.ncorr)
end

# Run with forcing ON
println("\nforce_tol = 1e-2 (ON):")
result_on = solve(prob, settings_on)
println("  Status: $(result_on.status)")
println("  IPM iters: $(result_on.ipm_niter)")
println("  Total CG: $(result_on.kkt_niter)")
println("  Per-iteration CG:")
println("  iter |      μ      | pred | corr | total")
println("  -----|-------------|------|------|------")
for (i, row) in enumerate(result_on.history)
    @printf("  %4d | %11.3e | %4d | %4d | %5d\n", i, row.μ, row.npred, row.ncorr, row.npred + row.ncorr)
end
...
println("\n" * "="^60)
println("Summary:")
println("  force_tol=0:    $(result_off.ipm_niter) IPM, $(result_off.kkt_niter) CG")
println("  force_tol=1e-2: $(result_on.ipm_niter) IPM, $(result_on.kkt_niter) CG")
cg_reduction = 100 * (1 - result_on.kkt_niter / result_off.kkt_niter)
@printf("  CG reduction: %.1f%%\n", cg_reduction)
